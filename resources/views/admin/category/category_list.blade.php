                                ...
                                <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                    ...
                                    @if(!empty($all_categories))
                                        @foreach($all_categories as $all_category)
                                            <tr>
                                                <td>{!! $all_category->category_type !!}</td>
                                                <td>{!! $all_category->category_name !!}</td>
                                                <td>{!! substr($all_category->category_description ,0,70)!!}</td>

                                                <td>

                                                    <a href="#" class="btn btn-simple btn-warning btn-icon" data-toggle="modal" onclick="get_category_data({{$all_category->id}})" data-target="#BrandModal"><i class="ti-pencil-alt"></i></a>

                                                    <a href="" class="btn btn-simple btn-info btn-icon del_brand"  id="del_brand_item" onclick="deleteCategory({{ $all_category->id }})"><i class="ti-trash"></i></a>
                        ...
                        <div class="row" style="padding: 10px">
                            ...
                            <div class="form-group">
                                <label class="control-label" for="category_info">
                                    Old Category Type<star>*</star>
                                </label>
                                <label for="category_name"></label><input class="form-control" type="text" name="old_category_type" id="old_category_type" readonly/>
                            </div>
                            <div class="form-group">
                                <label for="">Select Category<star>*</star></label>
                                <select  title="-" class="selectpicker"  data-style="btn-dark btn-block" data-size="4" name="category_type" id="category_type"  >
